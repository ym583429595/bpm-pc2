<template>
  <div class="">
    <!-- <iframe  v-show="showIframe" src='http://www.baidu.com' width=1200 height=800></iframe> -->
    ...
    <ul class="table-bagde" :class="{tabelOneLine: oneLine}">
      <li v-for="item in toDoListTypesGroup" @click="filterTitle(item.ProcessName)">
    ...
    <el-table :data="todoData" stripe v-loading="loading" element-loading-text="拼命加载中..." style="width: 100%" height="443" @row-click="rowClick" @sort-change="tableSortChange"  @cell-mouse-enter="handleMouseEnter" @cell-mouse-leave="handleMouseLeave">
      <el-table-column show-overflow-tooltip prop="Summary" label="流程主题" width="240"></el-table-column>
      ...
      <el-table-column prop="ExceedTime" :formatter="dateFormat" label="过期时间" width="130">
      </el-table-column>
      <el-table-column  label="操作" width="200">
        <template slot-scope="scope">
            ...
            <el-popover trigger="click" popper-class="agreePopover" ref="agreePopover" placement="bottom-end" width="160" @show="showAgreePop(scope.row)">
              <p>注意：请进入流程详情页处理{{scope.row.end_time}}</p>
              <div style="text-align: right; margin: 0">
                <el-button size="mini" type="text" @click="delPopoverCancel(scope.$index)">取消</el-button>
                <el-button type="primary" size="mini" @click="delPopoverOK(true, scope.row)">确定</el-button>
              </div>
            ...
            <el-button type="text" v-popover:agreePopover size="mini" >通过</el-button>
            <el-button type="text"  size="mini" @click='delOption(scope.row)'>驳回</el-button>
        </template>
      </el-table-column>
    </el-table>
...
Vue.use(Loading);
  ...
  export default {
    ...
    props: ['keyword'],
    computed: {
      ...mapGetters(['todoProcessData', 'toDoListTypesGroup'])
    },
    mounted() {
      this.getInitData();
    },
    methods: {
      getInitData() {
        ...
        this.$store.dispatch('getToDoListTypesGroup', this.typesData).then(() => {
      ...
      getTableData() {
        ...
        this.$store.dispatch('getTodoProcess', this.proData).then((res) => {
          this.$store.commit('get_todo_process', res.List);
          this.todoData = this.$store.state.bpm.todoProcess.todoProcessData;
      ...
      refresh() {
        this.$store.dispatch('getToDoListTypesGroup', this.typesData);
        this.getTableData();
      },
      // 过滤todoProcessData的副本，实现过滤流程类型
      filterTitle(filter) {
        if (filter !== 'Whole') {
          this.proData.processType = filter;
        } else {
          this.proData.processType = '';
        }
        this.getTableData();
      },
      toSearch() {
      ...
      agree(index, row) {
        console.log(row);
        // const approveData = {};
        // this.$store.dispatch('toDoListApprove', approveData);
      },
      notAgree(index, row) {
      ...
      delOption(row) {
        ...
        this.$prompt('请输入驳回原因', '提示', {
            ...
            data.comment = value;
            this.$store.dispatch('toDoListReject', data).then(() => {
               this.refresh();
            });
          });
      },
