<template>
  <div class="message">
    ...
    <ul class="table-bagde" :class="{tabelOneLine: oneLine}">
      <li v-for="item in myrequestDataListTypesGroup" @click="filterTitle(item.ProcessName)">
    ...
    <!-- table列表区 -->
    <el-table :data="requestData" stripe v-loading="loading" element-loading-text="拼命加载中..." style="width: 100%" height="443">
...
Vue.use(Loading);
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['myrequestProcessData', 'myrequestDataListTypesGroup'])
    },
    mounted() {
      this.getInitData();
    },
    methods: {
      getInitData() {
        ...
        this.$store.dispatch('getMyrequestListTypesGroup', this.typesData).then(() => {
          // this.typesData.status = true;
        });
        this.getTableData();
        setTimeout(() => {
          this.requestData = this.myrequestProcessData;
          this.loading = false;
        }, 1000);
      },
      getTableData() {
        ...
        this.$store.dispatch('getMyrequestProcess', this.proData).then((res) => {
          this.$store.commit('get_myrequest_process', res.List);
          this.requestData = this.$store.state.bpm.history.myrequestProcessData;
      ...
      refresh() {
        this.$store.dispatch('getMyrequestListTypesGroup', this.typesData);
        this.getTableData();
      },
