<template>
  <div class="message">
    ...
    <el-tabs v-model="tabName" >
        <el-tab-pane label="历史流程（BPM）" name="bpm">

    <div v-show='!loading'>
    <!-- 筛选按钮区域 -->
    <ul class="table-bagde" :class="{tabelOneLine: oneLine}">
      <li v-for="item in myapprovalDataListTypesGroup" @click="filterTitle(item.ProcessName)">
    ...
    <!-- table列表区 -->
    <el-table :data="approvalData" stripe v-loading="loading" element-loading-text="拼命加载中..." style="width: 100%" height="443">
      <el-table-column show-overflow-tooltip prop="Summary" label="流程名称" width="300"></el-table-column>
      <el-table-column prop="ProcessNameCN" show-overflow-tooltip label="流程类型" width="200"></el-table-column>
     <!--  <el-table-column prop="ApplicantName" show-overflow-tooltip label="启动用户" width="120"></el-table-column>-->
      <el-table-column prop="ActivityNameCN" show-overflow-tooltip resizable label="当前过程" width="100"></el-table-column>
      <el-table-column prop="BarCode" show-overflow-tooltip label="流程编号" width="240"></el-table-column>
      <el-table-column prop="CreateTime" :formatter="dateFormat" label="发起时间" width="150">
      </el-table-column>
      <el-table-column prop="UpdateTime" :formatter="dateFormat" label="最后更新时间" width="150">
      </el-table-column>
...
Vue.use(Loading);
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['myapprovalProcessData', 'myapprovalDataListTypesGroup'])
    },
    mounted() {
      this.getInitData();
    },
    methods: {
      getInitData() {
        ...
        this.$store.dispatch('getMyapprovalListTypesGroup', this.typesData).then(() => {
          // this.typesData.status = true;
        });
        this.getTableData();
        setTimeout(() => {
          this.approvalData = this.myapprovalProcessData;
          this.loading = false;
        }, 1000);
      },
      getTableData() {
        ...
        this.$store.dispatch('getMyapprovalProcess', this.proData).then((res) => {
          this.$store.commit('get_myapproval_process', res.List);
          this.approvalData = this.$store.state.bpm.history.myapprovalProcessData;
      ...
      refresh() {
        this.$store.dispatch('getMyapprovalListTypesGroup', this.typesData);
        this.getTableData();
      },
