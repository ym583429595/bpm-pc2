<template>
  <div class="message">
    ...
    <el-tabs v-model="tabName" >
        <el-tab-pane label="消息中心（BPM）" name="bpm">

    <div v-show='!loading'>
    <!-- 筛选按钮区域 -->
    <ul class="table-bagde" :class="{tabelOneLine: oneLine}">
      <li v-for="item in messageDataListTypesGroup" @click="filterTitle(item.ProcessName)">
    ...
    <!-- table列表区 -->
    <el-table :data="messageData" stripe v-loading="loading" element-loading-text="拼命加载中..." style="width: 100%" height="443">
...
Vue.use(Loading);
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['messageProcessData', 'messageDataListTypesGroup'])
    },
    mounted() {
      this.getInitData();
    },
    methods: {
      getInitData() {
        ...
        this.$store.dispatch('getMsgListTypesGroup', this.typesData).then(() => {
          // this.typesData.status = true;
        });
        this.getTableData();
        setTimeout(() => {
          this.messageData = this.messageProcessData;
          this.loading = false;
        }, 1000);
      },
      getTableData() {
        ...
        this.$store.dispatch('getMsgProcess', this.proData).then((res) => {
          this.$store.commit('get_message_process', res.List);
          this.messageData = this.$store.state.bpm.message.messageProcessData;
      ...
      refresh() {
        this.$store.dispatch('getMsgListTypesGroup', this.typesData);
        this.getTableData();
      },
